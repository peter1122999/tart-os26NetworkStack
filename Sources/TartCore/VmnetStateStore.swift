...
public struct VmnetStateEntry {
  public var config: VmnetConfig
  public var macAddress: String?
  public var runtimeState: VmnetRuntimeState
}
...
public struct EndpointMapping: Equatable {
  public let vmName: String
  public let macAddress: String?
  public let ipAddress: String?
  public let subnet: CIDRBlock?
}
...
public protocol VmnetPolicyProvider {
...
public actor VmnetStateStore {
  public static let shared = VmnetStateStore()

  private var entries: [String: VmnetStateEntry] = [:]

  public init() {}

  public func publish(vmName: String, config: VmnetConfig, macAddress: String?) {
  ...
  public func update(_ state: VmnetRuntimeState) {
  ...
  public func recordResolvedIP(vmName: String, ipAddress: String) {
    guard var entry = entries[vmName], let mac = entry.macAddress else { return }
    entry.runtimeState.lastKnownIPByMAC[mac] = ipAddress
    entries[vmName] = entry
  }

  public func remove(vmName: String) {
    entries.removeValue(forKey: vmName)
  }

  public func entry(for vmName: String) -> VmnetStateEntry? {
    entries[vmName]
  }

  public func runtimeState(for vmName: String) -> VmnetRuntimeState? {
    entries[vmName]?.runtimeState
  }

  public func config(for vmName: String) -> VmnetConfig? {
    entries[vmName]?.config
  }

  public func allEntries() -> [String: VmnetStateEntry] {
    entries
  }
}

extension VmnetStateStore: VmnetPolicyProvider {
  public func activeSubnets() async -> [CIDRBlock] {
    Array(Swift.Set(entries.values.compactMap { $0.runtimeState.actualSubnet ?? $0.config.subnet }))
  }

  public func vmIPAddresses() async -> [String: String] {
  ...
  public func endpointMappings() async -> [EndpointMapping] {
