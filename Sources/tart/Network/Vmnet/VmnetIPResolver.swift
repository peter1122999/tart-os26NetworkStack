...
enum IPResolutionTier: String, Codable, CaseIterable, Equatable {
...
struct IPResolutionAttempt: Equatable {
  let strategy: IPResolutionTier
  let succeeded: Bool
  let detail: String
}
...
struct IPResolutionOutcome {
  let address: IPv4Address?
  let attempts: [IPResolutionAttempt]

  var succeeded: Bool { address != nil }
}
...
struct ARPScopedVmnetRuntimeLookup: VmnetRuntimeLookup {
  func lookupIPAddress(forMAC mac: String, scopedTo subnet: CIDRBlock?) throws -> IPv4Address? {
...
/// agent). Abstracted so `VmnetIPResolver` doesn't have to depend on `IP.resolveIP`
/// directly, keeping this file testable without touching the filesystem or a real socket.
protocol LegacyIPResolving {
...
struct VmnetIPResolver {
  let runtimeLookup: VmnetRuntimeLookup
  let legacyResolver: LegacyIPResolving

  init(runtimeLookup: VmnetRuntimeLookup = ARPScopedVmnetRuntimeLookup(), legacyResolver: LegacyIPResolving = DefaultLegacyIPResolver()) {
  ...
  static func detectInconsistency(_ attempts: [IPResolutionAttempt]) -> String? {
